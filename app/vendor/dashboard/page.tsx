    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 md:py-14">
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-gray-200 pb-8">
          <div>
            <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight mb-1">Welcome Back, {displayName}</h1>
            <p className="text-gray-400 font-normal text-sm">Manage your products and track your store performance.</p>
          </div>
          <button onClick={() => setShowAddModal(true)} className="bg-gray-900 hover:bg-black text-white px-8 py-4 rounded font-bold transition-all duration-200 ease-in-out shadow-md hover:shadow-xl hover:shadow-gray-900/20 hover:-translate-y-0.5 active:scale-95 flex items-center gap-2 w-full md:w-auto justify-center uppercase tracking-wider text-xs">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-64 shrink-0">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sticky top-28">
              <nav className="space-y-1">
                <button onClick={() => setActiveTab('overview')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all duration-200 ${activeTab === 'overview' ? 'bg-red-50 text-red-600' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>
                  Overview
                </button>
                <button onClick={() => setActiveTab('products')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all duration-200 ${activeTab === 'products' ? 'bg-red-50 text-red-600' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}>
                ...
                <button onClick={() => setActiveTab('orders')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all duration-200 ${activeTab === 'orders' ? 'bg-red-50 text-red-600' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
                  Orders
                  <span className={`ml-auto py-0.5 px-2 rounded-full text-xs font-bold ${activeTab === 'orders' ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-700'}`}>12</span>
                </button>
                <button onClick={() => setActiveTab('payouts')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all duration-200 ${activeTab === 'payouts' ? 'bg-red-50 text-red-600' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}>
          ...
          <div className="flex-1 space-y-8">
            ...
            {activeTab === 'overview' && (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {/* Stat Cards */}
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-lg hover:shadow-gray-200/60 hover:-translate-y-1 hover:scale-[1.02] transition-all duration-200 ease-out cursor-default">
                    <p className="text-gray-400 font-medium text-[10px] mb-2 uppercase tracking-widest">Total Revenue</p>
                    <div className="flex items-end justify-between">
                      <h3 className="text-3xl font-extrabold text-gray-900 tracking-tight">₹{stats.revenue.toLocaleString()}</h3>
                      <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100">12.5%</span>
                    </div>
                  </div>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-lg hover:shadow-gray-200/60 hover:-translate-y-1 hover:scale-[1.02] transition-all duration-200 ease-out cursor-default">
                    <p className="text-gray-400 font-medium text-[10px] mb-2 uppercase tracking-widest">Active Orders</p>
                    <div className="flex items-end justify-between">
                      <h3 className="text-3xl font-extrabold text-gray-900 tracking-tight">{stats.orders}</h3>
                      <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100">4.1%</span>
                    </div>
                  </div>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-lg hover:shadow-gray-200/60 hover:-translate-y-1 hover:scale-[1.02] transition-all duration-200 ease-out cursor-default">
                    <p className="text-gray-400 font-medium text-[10px] mb-2 uppercase tracking-widest">Store Views</p>
                    <div className="flex items-end justify-between">
                      <h3 className="text-3xl font-extrabold text-gray-900 tracking-tight">{stats.views.toLocaleString()}</h3>
                      <span className="text-[10px] font-bold text-red-500 bg-red-50 px-2 py-0.5 rounded-md border border-red-100">1.2%</span>
                    </div>
                  </div>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-lg hover:shadow-gray-200/60 hover:-translate-y-1 hover:scale-[1.02] transition-all duration-200 ease-out cursor-default">
                    <p className="text-gray-400 font-medium text-[10px] mb-2 uppercase tracking-widest">Average Rating</p>
                    <div className="flex items-end justify-between">
                      <div className="flex items-center gap-2">
                        <h3 className="text-3xl font-extrabold text-gray-900 tracking-tight">{stats.rating}</h3>
                        <div className="flex text-yellow-400">
                          <svg className="w-5 h-5 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                        </div>
                      </div>
                      <span className="text-[10px] font-medium text-gray-400">142 reviews</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-300">
                  <div className="flex items-center justify-between mb-10">
                    <div>
                      <h3 className="font-extrabold text-gray-900 text-xl tracking-tight">Revenue Analytics</h3>
                      <p className="text-gray-400 font-normal text-xs mt-1">Earnings Performance (INR)</p>
                    </div>
                    <span className="text-xs font-bold text-gray-900 bg-gray-100 px-3 py-1.5 rounded-lg border border-gray-200">This Week</span>
                  </div>
                  <div className="h-72 border-b-2 border-gray-100 relative flex items-end justify-between px-4 pb-2 pt-10">
                    {/* Y-axis guide lines */}
                    {[25, 50, 75, 100].map(line => (
                      <div key={line} className="absolute left-0 right-0 border-t border-dashed border-gray-100" style={{ bottom: `${line}%` }} />
                    ))}
                    {[45, 75, 52, 95, 68, 100, 88].map((h, i) => (
                      <div key={i} className="w-[11%] rounded-t-lg transition-all duration-200 ease-out relative group cursor-pointer hover:scale-[1.05] hover:brightness-110 hover:shadow-lg hover:shadow-emerald-200/40" style={{ height: `${h}%`, background: 'linear-gradient(to top, #059669, #34d399)' }}>
                        <div className="absolute -top-14 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-[11px] font-bold py-2 px-4 rounded-lg opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100 transition-all duration-200 shadow-2xl whitespace-nowrap z-20 pointer-events-none">
                          <div className="flex items-center gap-1">₹{(h * 1230).toLocaleString()}</div>
                          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 rotate-45 w-2 h-2 bg-gray-900"></div>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="flex justify-between mt-5 text-[11px] font-semibold text-gray-500 px-4">
                    {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => (
                      <span key={day} className="w-[11%] text-center">{day}</span>
                    ))}
                  </div>
                </div>
              </>
            ...
            {activeTab !== 'overview' && activeTab !== 'products' && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center h-64 flex flex-col items-center justify-center">

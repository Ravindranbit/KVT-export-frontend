                ...
                <div key={order.id} className="px-5 py-3 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden">
                      <img src={order.items[0]?.image || '/placeholder.png'} alt="" className="w-7 h-7 rounded object-cover" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-gray-900">{order.id}</p>
                      <p className="text-[11px] text-gray-400">{order.userName} · {order.createdAt ? new Date(order.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '-'}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
            ...
            {topProducts.length > 0 ? (
              <div className="space-y-2.5">
                {topProducts.map((product, i) => (
                  <div key={product.id || i} className="flex items-center gap-2.5">
                    ...
                    <img src={product.imageUrl || product.image || '/placeholder.png'} alt="" className="w-8 h-8 rounded-lg object-cover border border-gray-100" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-gray-900 truncate">{product.name}</p>
                      <p className="text-[10px] text-gray-400">Stock: {product.stock ?? '—'}</p>
                    </div>
                    <span className="text-xs font-bold text-gray-900">₹{Number(product.price || 0).toLocaleString()}</span>
                  </div>
                ))}
              </div>
            ) : (
